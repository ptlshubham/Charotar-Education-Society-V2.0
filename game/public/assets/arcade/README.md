...
| _(canopy)_ | No folder of its own; uses the jungle terrain, characters and props with its forest route and backdrop |
| `shared/` | Retained original fallback sprites |

Runtime WebP images use semantic names such as `player.webp`, `beetle.webp`, `flyer.webp`, `plant.webp`, `walker.webp`, `pickups.webp`, `blocks.webp`, `props.webp` and `tiles.webp`. Ice's snails are inside `pickups.webp`; Village has no plant enemy sheet.
...
The loader removes the border-connected black matte and builds transparent, consistently anchored runtime atlases. Source image bytes remain intact. Terrain and pickup frames can sample several sheets in the same theme. Keep dimensions and layouts unchanged for a direct replacement; update region metadata if the export layout changes.
