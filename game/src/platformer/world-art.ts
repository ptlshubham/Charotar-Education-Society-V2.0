...
const file = (theme: Theme, name: string): string => `assets/arcade/${theme}/${name === 'terrain' ? 'tiles' : name}.png`;
...
const strip = (theme: Theme, name: 'player' | 'beetle' | 'flyer' | 'plant' | 'walker', width: number, height: number, count: number,
  ...
  notes: ['Supplied artwork. Black matte is keyed at load; source PNG bytes stay intact.', 'Frame mapping follows the supplied poses; missing poses reuse an existing frame.'],
});
...
for (const theme of Object.keys(pickups) as Theme[]) {
  ...
  const star: AtlasFrame = theme === 'volcano' ? region([1783,173,350,346]) : theme === 'village' ? region([537,219,358,379])
    : { file: 'assets/arcade/jungle/rewards.png', rect: [1086,0,543,724] };
  const sign: AtlasFrame = theme === 'ice' ? cell(theme, 'props', [870,320,142,215]) : theme === 'volcano' ? cell(theme, 'props', [1205,90,225,430])
    : theme === 'village' ? cell(theme, 'props', [1040,190,165,305]) : { file: 'assets/arcade/jungle/rewards.png', rect: [1629,0,543,724] };
  commons.push({
...
const props: Record<Theme, readonly AtlasFrame[]> = {
  desert: [cell('desert','props',[50,490,260,300]), {file:'assets/arcade/jungle/rewards.png',rect:[1629,0,543,724]}, cell('desert','blocks',[1165,215,270,545]),
...
const tileCells: Record<Theme, readonly AtlasFrame[]> = {
  ...
  ice: [cell('ice','terrain',[73,149,782,226],[.3,0,.3,1],'fill'), cell('ice','terrain',[55,407,243,519],[.15,.3,.65,.4],'fill'), cell('ice','blocks',[242,480,187,210],undefined,'fill'), cell('ice','blocks',[39,480,169,210],undefined,'fill'), cell('ice','blocks',[463,480,192,210],undefined,'fill'), cell('ice','blocks',[913,450,278,240],[.33,0,.34,1],'fill'), {file:'assets/arcade/jungle/tiles.png',rect:[397,397,396,396],fit:'fill'}, {file:'assets/arcade/jungle/tiles.png',rect:[794,397,396,396],fit:'fill'}, props.ice[2], props.ice[2]],
...
export const IMPORTED_SPRITES: readonly SpriteSlot[] = [
  ...
  { slot: 'castle-beetle', file: 'assets/arcade/castle/knight.png', frameW: 24, frameH: 24, cols: 7, rows: 1, frames: enemyFrames, pixelArt: true,
  ...
  { slot: 'castle-player', file: 'assets/arcade/castle/player.png', frameW: 24, frameH: 32, cols: 7, rows: 1, frames: playerFrames, pixelArt: true,
