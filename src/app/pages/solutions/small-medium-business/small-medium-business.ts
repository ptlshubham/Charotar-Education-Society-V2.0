...
export class SmallMediumBusiness {
  ...
  readonly dashboardNav = ['Overview', 'Customers', 'Invoices', 'Projects', 'Tasks', 'Reports', 'Team', 'Integrations', 'Settings'];
  readonly dashboardStats: ReadonlyArray<{ label: string; value: string; change: string; down: boolean }> = [
  ...
  readonly recentActivities: ReadonlyArray<{ icon: string; color: string; text: string; time: string }> = [
    ...
    { icon: 'folder', color: '#E8A33D', text: "Project 'Website Redesign' updated", time: '1h ago' },
  ...
  readonly features: ReadonlyArray<{ icon: string; title: string; desc: string }> = [
    { icon: 'users', title: 'Customer Management', desc: 'Organize customer data, track interactions, and build stronger relationships.' },
    { icon: 'invoice', title: 'Sales & Invoicing', desc: 'Create quotes, send invoices, and get paid faster with automated workflows.' },
    { icon: 'clipboard', title: 'Project & Task Management', desc: 'Plan projects, assign tasks, and keep your team aligned and productive.' },
    { icon: 'receipt', title: 'Expense Tracking', desc: 'Track expenses, categorize them and manage your cash flow effectively.' },
    { icon: 'pie', title: 'Reports & Insights', desc: 'Get real-time insights and reports to make smarter decisions and grow confidently.' },
    { icon: 'team', title: 'Team Collaboration', desc: 'Collaborate with your team in one place with role-based access and activity tracking.' },
  ];
